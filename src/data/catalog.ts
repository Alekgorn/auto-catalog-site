...
/**
 * Класс магнитолы — то, чем один уровень отличается от другого.
 * Справочник правится в админке, товары ссылаются на него ключом.
 */
export interface ProductLevel {
  /** Ключ, по которому товар ссылается на класс. После создания не меняется */
  id: string;
  /** Название класса: «Базовый уровень», «Флагман» */
  title: string;
  /** Платформы уровня — подпись под названием: «TS105, G85» */
  series: string;
  /** Одно предложение о том, кому подходит */
  summary: string;
  /** Характеристики уровня — по ним видно разницу */
  specs: [string, string][];
  /** К чему подходит: короткая строка о машинах и задачах */
  suits: string;
  /** Свои строки, если стандартных полей не хватило */
  extra: [string, string][];
  /** Показывать класс покупателю */
  active: boolean;
}