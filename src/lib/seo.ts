 ...
 * считает цену протухшей и может не показать её в сниппете.
 *
 * Дата округлена до первого числа — и это важно. Раньше здесь стояло
 * «сегодня плюс месяц», то есть значение менялось каждые сутки и
 * попадало в разметку КАЖДОГО товара. Любая пересборка переписывала
 * все полторы тысячи страниц только из-за сдвига этой даты, хотя ни
 * цена, ни описание не трогались. Теперь в течение месяца дата
 * постоянная, а запас до неё — от месяца до двух, чего поисковику
 * достаточно: ему нужна дата в будущем, а не точность до дня.
 */
