...
/**
 * Карточка витрины «Что мы уже собрали».
 *
 * Не лента заказов, а редактура: показываем не самое свежее, а самое
 * понятное. Состав храним ссылками на товары — названия и цены берутся
 * из каталога живыми, иначе витрина обещала бы вчерашнюю стоимость.
 */
export interface ShowcaseKit {
  /** Своё имя карточки — обычно машина: «Kia Rio 2017» */
  title: string;
  /** Адреса товаров в комплекте */
  ids: string[];
  /** Фото установки. Пусто — подставим снимок первого товара */
  image?: string;
  /** «Под заказ, 5–7 дней» — срок объявляем до оплаты, а не после */
  term?: string;
  /**
   * Ваш комментарий к сборке: «Авто со штатным усилителем», «Кнопки на
   * руле сохранены». Объясняет, почему подобрано именно так — из одних
   * названий товаров это не читается.
   */
  note?: string;
  /**
   * Что именно собрали: «Магнитола Topway TS105 9″ с рамкой и проводкой».
   * Заголовок называет машину, и без этой строки выходило, будто мы
   * собрали саму Ладу Весту, а не комплект для неё.
   */
  kitName?: string;
  /** Ссылка на готовый комплект: /sborka?p=... */
  href?: string;
}

/** Пусто по умолчанию: блок появляется, только когда есть что показать. */
export const DEFAULT_SHOWCASE: ShowcaseKit[] = [];
