...
export interface ParsedQuery {
  ...
  priceOrder?: 'asc' | 'desc';
  /**
   * Что нашлось в поисковом индексе: слово → коды товаров, в описании
   * которых оно встречается. Описания вынесены из каталога ради его
   * веса, и без этого поиск не находил бы товар по словам, которых нет
   * в названии. Пусто, пока индекс не подъехал, — поиск работает и без
   * него, просто по названию и характеристикам.
   */
...
const scoreProduct = (item: Indexed, q: ParsedQuery): SearchHit | null => {
  ...
  q.stems.forEach((s, i) => {
    ...
    /*
     * Описания в каталог не попадают — они весят два мегабайта и лежат
     * отдельно. Вместо них смотрим в поисковый индекс: он знает, в каких
     * товарах слово встречается, и весит впятеро меньше. Тот же вес, что
     * у совпадения по тексту выше: сигнал слабее названия, но настоящий.
     */
    const fromIndex = q.indexHits?.get(word);
    if (fromIndex?.has(item.product.id)) {
      score += 18;
      wordHits += 1;
      return;
    }
   ...
   */
  /*
   * Исключение — слово нашлось в поисковом индексе описаний.
   *
   * Индекс отвечает на другой вопрос, чем поиск по буквам: не «встретилась
   * ли подстрока», а «есть ли ровно это слово в описании товара». Такое
   * совпадение не случайно — редкие слова вроде «восьмиядерный» или
   * «ABS-пластик» в названиях не встречаются вовсе, и глухой отказ здесь
   * означал бы, что по ним не найти ничего.
   */
  const foundInDescription = q.words.some((w) =>
    q.indexHits?.get(w)?.has(item.product.id),
  );
...
export const smartSearch = (
  ...
  /*
   * Подмешиваем поиск по описаниям. Сами описания в каталог не попадают
   * (два мегабайта на каждой странице), поэтому спрашиваем индекс: в
   * каких товарах встречается каждое слово запроса. Если индекс ещё не
   * загрузился, поиск отработает по названию и характеристикам.
   */
  const indexHits = new Map<string, Set<string>>();
  q.words.forEach((word, i) => {
    /* Сначала слово как есть, потом его основа: человек пишет
       «ударопрочный», а в описании «ударопрочного» */
    const ids = idsWithWord(word) ?? idsWithWord(q.stems[i] ?? '');
    if (!ids) return;
    /*
     * Слово, которое есть больше чем у половины каталога, ничего не
     * отбирает — это стоп-слово по факту, а не по словарю («ABS»,
     * «установка», «магнитолы»). Учитывать его вредно: в запросе
     * «ударопрочный ABS» оно перевесило бы редкое слово и превратило
     * точную выдачу в весь каталог.
     */
    if (ids.size > products.length / 2) return;
    indexHits.set(word, ids);
  });
  ...
  /*
   * Ни один товар не совпал названием — слова нашлись лишь в описаниях.
   * Когда таких товаров МНОГО, выдача и правда выглядит случайной:
   * слово вроде «установка» мелькает у половины каталога.
   *
   * Но если по описанию нашлась горстка товаров — это как раз точное
   * попадание: человек ищет «восьмиядерный» или «ABS-пластик», таких
   * слов в названиях нет вовсе. Раньше здесь стояло глухое «ничего не
   * найдено», и редкие запросы уходили в пустоту.
   */
  const byIndex = result.filter((h) =>
    q.words.some((w) => q.indexHits?.get(w)?.has(h.product.id)),
  ).length;
  /* Слово из индекса — не случайное совпадение букв, а настоящее слово
     из описания. Пустой ответ здесь был бы неправдой: товары есть.
     Отсекаем, только если слово размазано по половине каталога — тогда
     оно ничего не отбирает и выдача бессмысленна */
  if (
    result.length > 30 &&
    result[0].score < 100 &&
    (byIndex === 0 || byIndex > products.length / 2)
  ) {
    return [];
  }
